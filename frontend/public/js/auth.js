  ...
  regForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        regError.textContent = data.msg || data.error || 'Registration failed';
      }
    } catch (err) {
  ...
  logForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (res.ok && data.token) {
        ...
        localStorage.setItem('role', data.user && data.user.role ? data.user.role : '');
        localStorage.setItem('name', data.user && data.user.name ? data.user.name : '');
        // Redirect after login
        window.location.href = '/dashboard.html';
      } else {
        logError.textContent = data.msg || data.error || 'Login failed';
      }
    } catch (err) {
